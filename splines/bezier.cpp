...
void display()
{
  ...
  if (points_count > 0)
  {
    ...
    if (points_count > 1)
    {
      if((first_entered == 0 && points_count % 4 == 0) || last_count + 3 == points_count){
        printf("new\n");
        first_entered = 1;
        last_count = points_count;
        for(int j = 0, k = 4; k < points_count; j = k - 1 , k += 3){
          printf("j = %d   k = %d\n", j , k);
          // fflush(stdout);

          float temp_control_points[4][2];
          for(int m = 0 , i = j; i < k; i++, m++){
            temp_control_points[m][0] = control_points[i][0];
            temp_control_points[m][1] = control_points[i][1];
          }


          for (i=0; i<RES; i++)
          {
            bezier(temp_control_points, 4 ,(float)i/(float)(RES-1));
          }
        }
